...
export function UIProvider({ children }) {
  const {
    events, flights, vessels, globeLoading,
    news, newsLoading,
    overview, financeLoading,
    fetchOverview, fetchPredictions,
    addAutoSignals, setAutoWatchlist,
  ...
  const [fullPageView, setFullPageView] = useState(null); // 'intel', 'trade', 'forecast', 'sitrep', 'signals'
  ...
  const [unviewedSignalCount, setUnviewedSignalCount] = useState(0);
    ...
    events.forEach(evt => {
      if (evt.severity === 'CRITICAL' && !toastedEventIds.current.has(evt.id)) {
        ...
        toast.addToast({
          type: 'CRITICAL',
          title: evt.title,
          subtitle: evt.country ? `REGION: ${evt.country.toUpperCase()}` : undefined
        });
      }
    });
  ...
  useEffect(() => {
    const handleKeyboard = (e) => {
      ...
      // View toggles
      if (key === 'g') { setViewMode(prev => prev === 'globe' ? 'map2d' : 'globe'); audio.playClick(); }
      if (key === 'h') { 
      ...
      if (key === 'f') { setShowFlights(prev => !prev); audio.playClick(); }
      if (key === 'c') { setShowCyber(prev => !prev); audio.playClick(); }
      if (key === 'r') { setShowRegions(prev => !prev); audio.playClick(); }
      if (key === 'm') { setShowHeatmap(prev => !prev); audio.playClick(); }
      if (key === 'v') { handleCommsToggle(); }

      // Tab switching
      if (key === '1') { setActiveTab('news'); audio.playClick(); }
      if (key === '2') { setActiveTab('finance'); audio.playClick(); }
      if (key === '3') { setActiveTab('predictions'); audio.playClick(); }
      if (key === '4') { setActiveTab('sitrep'); audio.playClick(); }
      if (key === '5') { setActiveTab('signals'); audio.playClick(); setUnviewedSignalCount(0); }

      // Time range
      if (e.altKey && key === '1') { e.preventDefault(); setTimeRange('1h'); }
      if (e.altKey && key === '2') { e.preventDefault(); setTimeRange('6h'); }
      if (e.altKey && key === '3') { e.preventDefault(); setTimeRange('24h'); }
      if (e.altKey && key === '4') { e.preventDefault(); setTimeRange('7d'); }
  ...
  useEffect(() => {
    ...
    // ─── Auto-Signal WebSocket Listeners ──────────────────────
    const unsub3 = onEvent('signal:autoGenerated', (payload) => {
      if (payload?.signals?.length > 0) {
        addAutoSignals(payload.signals);
        setUnviewedSignalCount(prev => prev + payload.signals.length);

        // Show SIGNAL toast for each generated signal
        payload.signals.forEach(sig => {
          toast.addToast({
            type: 'SIGNAL',
            title: `${sig.signal} ${sig.ticker} — ${sig.confidence}% Confidence`,
            subtitle: `AUTO-SIGNAL • ${payload.triggerEvent?.title?.substring(0, 50) || 'CRITICAL EVENT'}`,
          });
        });

        audio.playBloop();

        // Auto-refresh signal history if tab is active
        fetchSignalHistory();
        fetchSignalStats();
      }
    });

    const unsub4 = onEvent('signal:watchlist', (payload) => {
      if (payload?.tickers?.length > 0) {
        setAutoWatchlist(payload);
      }
    });
  ...
  useEffect(() => { 
    ...
    fetchSignalHistory();
    fetchSignalStats();
  }, [fetchOverview, fetchPredictions, fetchSignalHistory, fetchSignalStats]);
...
return events.filter(e => {
  ...
  const filteredFlights = useMemo(() => {
    ...
    return flights.filter(f => {
      ...
      if (flightCategory === 'isr') return t.includes('uav') || t.includes('recon') || t.includes('awacs') || t.includes('strategic') || t.includes('high') || t.includes('maneuv');
      if (flightCategory === 'transport') return t.includes('heavy') || t.includes('large') || t.includes('transport');
      if (flightCategory === 'heli') return t.includes('rotor') || t.includes('heli');
      if (flightCategory === 'tanker') return t.includes('tanker') || t.includes('refuel');
      return !['isr','transport','heli','tanker'].some(cat => {
        if (cat === 'isr') return t.includes('uav') || t.includes('recon') || t.includes('awacs') || t.includes('strategic') || t.includes('high');
        if (cat === 'transport') return t.includes('heavy') || t.includes('large') || t.includes('transport');
        if (cat === 'heli') return t.includes('rotor') || t.includes('heli');
        if (cat === 'tanker') return t.includes('tanker') || t.includes('refuel');
        return false;
      });
    });
  ...
  const filteredVessels = useMemo(() => {
    if (!vessels) return [];
    if (vesselCategory === 'all') return vessels;
    return vessels.filter(v => (v.vesselType || '').toLowerCase() 
    
    === vesselCategory.toLowerCase());
  }, [vessels, vesselCategory]);

  const missionMetrics = useMemo(() => {
    const allIntel = [...events, ...news];
    return {
      active: allIntel.filter(n => n.severity === 'CRITICAL').length,
      tensions: allIntel.filter(n => n.severity === 'HIGH' || n.severity === 'MEDIUM').length
    };
  }, [events, news]);
  ...
  const tabBadges = useMemo(() => ({
    ...
    signals: unviewedSignalCount,
  }), [news, events, unviewedSignalCount]);
  ...
  const handleRegionClick = useCallback((regionName) => {
    const coords = {
      'Middle East': { lat: 29, lng: 42 }, 'Europe': { lat: 50, lng: 15 },
      'Asia-Pacific': { lat: 25, lng: 115 }, 'Americas': { lat: 10, lng: -80 },
      'Africa': { lat: 5, lng: 20 },
    };
  ...
  const value = useMemo(() => ({
    ...
    showCyber, setShowCyber,
    showRegions, setShowRegions,
    showHeatmap, setShowHeatmap,
    showTension, setShowTension,
    viewMode, setViewMode,
    activeFilters, setActiveFilters, handleFilterToggle,
    timeRange, setTimeRange,
    minTime, maxTime, scrubTime, setScrubTime,
    activeTab, setActiveTab,
    isCommsActive, handleCommsToggle,
    leftPanelVisible, setLeftPanelVisible,
    rightPanelVisible, setRightPanelVisible,
    flightCategory, setFlightCategory,
    vesselCategory, setVesselCategory,
    wargameEvent, setWargameEvent,
    isOmniOpen, setIsOmniOpen,
    isShortcutsOpen, setIsShortcutsOpen,
    fullPageView, setFullPageView,

    // Computed / Handlers
    defconLevel, missionMetrics, tabBadges,
    filteredEvents, filteredFlights, filteredVessels,
    handleCountryClick, handleTickerEventClick, handleRegionClick,
    unviewedSignalCount, setUnviewedSignalCount,

    // Audio Tooling
    audio
  }), [
    activeTab, isCommsActive, handleCommsToggle, leftPanelVisible, setLeftPanelVisible, rightPanelVisible, setRightPanelVisible, flightCategory, wargameEvent, isOmniOpen, isShortcutsOpen, fullPageView,
    defconLevel, missionMetrics, tabBadges, filteredEvents, filteredFlights, handleCountryClick, handleTickerEventClick, handleRegionClick,
    unviewedSignalCount, setUnviewedSignalCount, showTension, setShowTension,
    audio
  ]);
